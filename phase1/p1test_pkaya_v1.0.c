...
int main() {
	...
	for (i = 0; i < 10; i++) {
		addokbuf("Entriamo nel for  \n");
		if ((q = allocPcb()) == NULL){
			addokbuf("allocPcb NuLl   \n");
			adderrbuf("ERROR: allocPcb(): unexpected NULL while insert   ");}
		switch (i) {
			case 3:
				addokbuf("Caso 3 switch  \n");
				q->priority=DEFAULT_PCB_PRIORITY;
				proc = q;
			break;
			case 4:
				addokbuf("caso 4 switch   \n");
				q->priority=MAX_PCB_PRIORITY;
				maxproc = q;
			break;	
			case 5:
				addokbuf("caso 5 switch   \n");
				q->priority=MIN_PCB_PRIORITY;
				minproc=q;
			break;
			default:
				addokbuf("default switch   \n");
				q->priority=DEFAULT_PCB_PRIORITY;
			break;
		}
		addokbuf("prima dell'insertProcQ  \n");
		insertProcQ(&qa, q);
		addokbuf("dopo l'insertProcQ  \n");
	}
	...
	/* make procp[1],procp[2],procp[4], procp[7] children of procp[0] */
	addokbuf("Inserting...   \n");
	...
	addokbuf("ERROR: insertBlocked() and removeBlocked() ok   \n");
