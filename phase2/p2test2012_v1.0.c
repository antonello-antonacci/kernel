...
void test() {	
	...
	p2state.status = p2state.status | STATUS_IEp | STATUS_INT_UNMASKED; //STATUS_IEp
...
void p2() {
  ...
  /* startp2 is initialized to 0. p1 Vs it then waits for p2 termination */
	...
	for (i = 1; i < LOOPNUM; i++){} //PARENTESI AGGIUNTE DA NOI
...
void p4bis() {
	...
	SYSCALL(TERMINATEPROCESS, -1, 0, 0);			/* terminate p4bis   */
